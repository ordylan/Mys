...
<head>
    ...
    <style>
        ...
        .manage-topic.dragging {
            opacity: 0.4;
        }

        .manage-subject-header {
            display: flex;
            align-items: center;
            gap: 10px;
            cursor: grab;
        ...
        .manage-subject-header.dragging {
            opacity: 0.4;
        }

        .manage-subject .topicsList {
            display: none;
        }

        .manage-subject.expanded .topicsList {
            display: block;
        }
...
<body>
    <div class="container">
        ...
        <div id="announcement" style="margin-bottom:15px;padding:10px;border:1px solid #ccc;border-radius:5px;background-color:#f0f8ff;text-align:center;font-size:14px;">
            ...
            Hi there! For cloud data sync, please goto <a href="ToDos.php">[任务表]</a> ! 
            <hr>
        ...
        <div class="button-group">
            ...
            <button id="cloudBtn" style="display: none;">Cloud(old)</button>
    ...
    <div id="cloudModal">
        <div class="cloud-modal-content">
            <span class="close" id="cloudClose">&times;</span>
            <h2>Cloud Archive</h2>
    ...
    <script>
        (function() {
            ...
            function getDefaultConfig() { //~~记得删除
                return {
                    id: 'main',
                    subjects: [{
                        key: 'mysubject',
                ...
                return new Promise((resolve, reject) => {
                    ...
                    request.onupgradeneeded = (e) => {
                        ...
                        if (!db.objectStoreNames.contains(KPS_STORE)) {
                            const store = db.createObjectStore(KPS_STORE, {
                                keyPath: 'uniqueId'
                            });
                        ...
                        if (!db.objectStoreNames.contains('DailyPlans')) {
                            const dailyStore = db.createObjectStore('DailyPlans', {
                                keyPath: 'id',
                                autoIncrement: true
                            });
                        ...
                        if (!db.objectStoreNames.contains('Announcements')) {
                            const annStore = db.createObjectStore('Announcements', {
                                keyPath: 'id',
                                autoIncrement: true
                            });
                            annStore.createIndex('pinnedAt', 'pinnedAt', {
                                unique: false
                            });
                        }

                        if (!db.objectStoreNames.contains('Flawless')) {
                            const flawlessStore = db.createObjectStore('Flawless', {
                                keyPath: 'id',
                                autoIncrement: true
                            });
                            flawlessStore.createIndex('date', 'date', {
                                unique: true
                            });
                        }
            ...
            async function renderTableAndUrgent() {
                ...
                for (let row = 0; row < maxRows; row++) {
                    ...
                    appConfig.subjects.forEach(subj => {
                        ...
                        const defId = subj.kps[row];
                        if (defId != null) {
                            const data = topicsMap[defId] || {
                                uniqueId: defId,
                                subject: subj.key,
                                name: 'Unknown',
                                clickCount: 0,
            ...
            async function displayAllTopicsView() {
                ...
                for (const subj of appConfig.subjects) {
                    ...
                    urgentDiv.appendChild(header);
                    const listDiv = document.createElement('div');
                    subj.kps.forEach(kpId => {
                        const t = topicsMap[kpId] || {
                            clickCount: 0,
                            name: 'Unknown'
                        };
                        const span = document.createElement('span');
            ...
            function syncEditingConfigFromDOM() {
                ...
                subjDivs.forEach((div, i) => {
                    ...
                    topicDivs.forEach(tDiv => {
                        ...
                        if (idSpan) {
                            const id = Number(idSpan.textContent.trim());
                            editingConfig.subjects[i].kps.push(id);
                            if (nameInput && window.manageTopicsMap) {
                                const name = nameInput.value.trim();
                                if (window.manageTopicsMap[id]) {
                                    window.manageTopicsMap[id].name = name;
                                } else {
                                    window.manageTopicsMap[id] = {
                                        uniqueId: id,
                                        subject: editingConfig.subjects[i].key,
                                        name: name,
                                        clickCount: 0,
                                        lastClicked: new Date(0).toISOString()
                                    };
            ...
            async function openManageModal() {
                ...
                const allTopics = await getAllTopics();
                window.manageTopicsMap = {};
                allTopics.forEach(t => window.manageTopicsMap[t.uniqueId] = t);
                renderManageUI();
            ...
            function renderManageUI() {
                ...
                <div class="topicsList" style="margin-top:8px;">
                   ${subj.kps.map((topicId, tIdx) => {
    const topicData = window.manageTopicsMap[topicId];
    const displayName = topicData ? topicData.name : 'New KPs';
    return `
        <div class="manage-topic" draggable="true" data-topic-index="${tIdx}">
            <span class="drag-handle">☰</span>
            <span class="topicIdDisplay" style="width:120px;">${topicId}</span>
            <input class="topicName" value="${displayName}" style="width:150px;">
    ...
    `;
            ...
            `;
                    ...
                    div.querySelector('.addTopic').onclick = () => {
                        ...
                        window.manageTopicsMap[newId] = {
                            uniqueId: newId,
                            subject: editingConfig.subjects[sIdx].key,
                            name: 'New KPs',
                            clickCount: 0,
                            lastClicked: new Date(0).toISOString()
                        };
                        editingConfig.subjects[sIdx].kps.push(newId);
                        renderManageUI();
                    };
            ...
            document.getElementById('saveConfigBtn').onclick = async () => {
                ...
                // 验证ID唯一性
                const allIds = editingConfig.subjects.flatMap(s => s.kps);
                ...
                for (const subj of editingConfig.subjects) {
                    for (const topicId of subj.kps) {
                        const newName = nameUpdates[topicId] || '';
                        const existing = existingMap[topicId];
                        if (existing) {
                    ...
                    try {
                        ...
                        openReq.onupgradeneeded = (e) => {
                            ...
                            if (!idb.objectStoreNames.contains(KPS_STORE)) {
                                const os = idb.createObjectStore(KPS_STORE, {
                                    keyPath: 'uniqueId'
                                });
                                os.createIndex('subject', 'subject');
                            }
                            if (!idb.objectStoreNames.contains(LOGS_STORE)) {
                                const os = idb.createObjectStore(LOGS_STORE, {
                                    keyPath: 'id'
                                });
                            ...
                            if (!idb.objectStoreNames.contains(CONFIG_STORE)) {
                                idb.createObjectStore(CONFIG_STORE, {
                                    keyPath: 'id'
                                });
                            }
                            if (!idb.objectStoreNames.contains('DailyPlans')) {
                                const dailyStore = idb.createObjectStore('DailyPlans', {
                                    keyPath: 'id',
                                    autoIncrement: true
                                });
                                dailyStore.createIndex('date', 'date', {
                            ...
                            if (!idb.objectStoreNames.contains('Announcements')) {
                                const annStore = idb.createObjectStore('Announcements', {
                                    keyPath: 'id',
                                    autoIncrement: true
                                });
                                annStore.createIndex('pinnedAt', 'pinnedAt', {
                            ...
                            if (!idb.objectStoreNames.contains('Flawless')) {
                                const flawlessStore = idb.createObjectStore('Flawless', {
                                    keyPath: 'id',
                                    autoIncrement: true
                                });
                                flawlessStore.createIndex('date', 'date', {
